import { Block, BlockPermutation, BlockType, BlockTypes, DimensionLocation, LocationOutOfWorldBoundariesError, Player, UnloadedChunksError } from "@minecraft/server";
import { PaintBlockType, PaintPositionedSnippet, PaintSnippet } from "../types/paint-snippet";
...
export class Paint {
    public static BLOCK_IDENTIFIER = "typidoyun:sketch";
    public base: PaintSnippet[];
    public positioned: PaintPositionedSnippet[];
    ...
    public addBases(...bases: PaintSnippet[]) {
        this.base.push(...bases);

        for (const base of bases) {
            if (typeof base.type === "string") {
                const type = BlockTypes.get(base.type);

                if (!type) throw new Error(`Block type "${base.type}" not found`);
            }
            else {
                const type = BlockTypes.get(base.type.id);

                if (!type) throw new Error(`Block type "${base.type.id}" not found`);
            }
        }
    ...
    public addPositioneds(...positioned: PaintPositionedSnippet[]) {
        this.positioned.push(...positioned);

        for (const base of positioned) {
            for (const blockType of base.heightMap) {
                if (typeof blockType === "string") {
                    const type = BlockTypes.get(blockType);
    
                    if (!type) throw new Error(`Block type "${blockType}" not found`);
                }
                else {
                    const type = BlockTypes.get(blockType.id);
    
                    if (!type) throw new Error(`Block type "${blockType.id}" not found`);
                }
            }
        }
    ...
    public static fromObject(paintJSON: PaintObject) {
        const paint = new Paint(paintJSON.name);
        paint.addBases(...paintJSON.base);
        paint.addPositioneds(...paintJSON.positioned);

        return paint;
    }

    public *paintShape(executor: Player, shape: Shape, origin: DimensionLocation) {
        ...
        if (snippetPercentSum > 100) return;
        if (positionedSnippetPercentSum > 100) return;
        ...
        const blockChangeMap = new Map<Block, PaintBlockType>();

        try {
            const blocks = Array.from(shape.getBlocks(originVector, origin.dimension));

            for (const block of blocks) {
                if (block.typeId !== Paint.BLOCK_IDENTIFIER) continue;
                ...
                if (!selectedSnippet) blockChangeMap.set(block, BlockTypes.get("minecraft:air")!);
                else blockChangeMap.set(block, selectedSnippet.type);
            }
            ...
            for (const block of blocks) {
                ...
                try {
                    for (let index = 0; index < selectedSnippet.heightMap.length; index++) {
                        ...
                        if (!above) break;
                        if (above.typeId !== "minecraft:air") break;
                        if (index !== 0) {
                            const bottomBlockType = selectedSnippet.heightMap[index - 1];
                            const bottomBlockTypeId = typeof bottomBlockType === "string" ? bottomBlockType : (bottomBlockType instanceof BlockPermutation ? bottomBlockType.type.id : bottomBlockType.id);
                            const actualBottomBlockType = blockChangeMap.get(bottomBlock);
                            
                            if (!actualBottomBlockType) break;

                            const actualBottomBlockTypeId = typeof actualBottomBlockType === "string" ? actualBottomBlockType : (actualBottomBlockType instanceof BlockPermutation ? actualBottomBlockType.type.id : actualBottomBlockType.id);
                            
                            if (actualBottomBlockTypeId !== bottomBlockTypeId) break;
                        }
            ...
            for (const [ block, blockType ] of blockChangeMap.entries()) {
                if (typeof blockType === "string") block.setType(blockType);
                else {
                    const blockPermutation = BlockPermutation.resolve(blockType.id, blockType.states);
            ...
            if (error instanceof UnloadedChunksError) {
                sendWarning(executor, "청크가 로딩되지 않았습니다.");
            } else throw error;
        }
