...
export class EraserComponent {
    ...
    onUse(eventData) {
        ...
        let shape = playerErasers.get(source) ?? playerErasers.set(source, new Cuboid()).get(source);
    ...
    onMineBlock(eventData) {
        ...
        system.runJob(function* () {
            while (queue.size > 0) {
                ...
                // current.clone.add([0, -1, -1]),
                // current.clone.add([1, -1, -1]),
                current.clone.add([1, 0, 0]), current.clone.add([0, 1, 0]), current.clone.add([0, 0, 1]), current.clone.add([-1, 0, 0]), current.clone.add([0, -1, 0]), current.clone.add([0, 0, -1]));
            }
            recorder.save();
        }());
    }
