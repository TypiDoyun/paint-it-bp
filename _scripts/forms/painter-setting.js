...
export const playerPaints = new Map();
export const playerPaintShapes = new Map();
export const getPlayerPaintShape = (player) => {
...
export const sendPainterSettingForm = (player, backFunction) => {
    return new Promise((resolve, reject) => {
        system.run(async () => {
            ...
            if (response.canceled && response.cancelationReason === FormCancelationReason.UserBusy)
                return resolve(sendPainterSettingForm(player));
            ...
            if (response.selection === 0) {
                ...
                return resolve(sendSelectPaintForm(player, sendPainterSettingForm));
            resolve(undefined);
        });
    });
};
const sendSelectPaintForm = (player, backFunction) => {
    return new Promise((resolve, reject) => {
        system.run(async () => {
            const actionForm = new ActionFormData()
            ...
            const response = await actionForm.show(player);
            if (response.canceled && response.cancelationReason === FormCancelationReason.UserBusy)
                return resolve(sendSelectPaintForm(player));
            if (response.selection === undefined)
                return resolve(backFunction?.(player));
            if (response.selection === 0)
                return resolve(sendSinglePaintForm(player, sendSelectPaintForm));
            else if (response.selection === 1)
                return resolve(sendMixPaintForm(player, sendSelectPaintForm));
            else if (response.selection === 2)
                return resolve(sendCustomPaintForm(player, sendSelectPaintForm));
            resolve(undefined);
        });
    });
};
const sendSinglePaintForm = (player, backFunction) => {
    return new Promise((resolve, reject) => {
        system.run(async () => {
            ...
            if (currentPaint)
                actionForm.body(`§l§6 > §r§f현재 페인트: ${currentPaint.name}`);
            singlePaints.forEach(paint => actionForm.button(paint.name));
            const response = await actionForm.show(player);
            if (response.canceled && response.cancelationReason === FormCancelationReason.UserBusy)
                return resolve(sendSinglePaintForm(player, backFunction));
            if (response.selection === undefined)
                return resolve(backFunction(player));
...
const sendMixPaintForm = (player, backFunction) => {
    return new Promise((resolve, reject) => {
        system.run(async () => {
            const actionForm = new ActionFormData()
            ...
            const response = await actionForm.show(player);
            if (response.canceled && response.cancelationReason === FormCancelationReason.UserBusy)
                return resolve(sendMixPaintForm(player, backFunction));
            if (response.selection === undefined)
                return resolve(backFunction(player));
            if (response.selection === 0) {
...
const sendCustomPaintForm = (player, backFunction) => {
    return new Promise((resolve, reject) => {
        system.run(async () => {
            const actionForm = new ActionFormData()
            ...
            const response = await actionForm.show(player);
            if (response.canceled && response.cancelationReason === FormCancelationReason.UserBusy)
                return resolve(sendCustomPaintForm(player, backFunction));
            if (response.selection === undefined)
                return resolve(backFunction(player));
            if (response.selection === 0) {
                const paint = singlePaints[0];
